...
msg['Subject'] = Header('坐标顶端功能测试报告', 'utf-8').encode()#主题

# 邮件正文是MIMEText:
msg.attach(MIMEText('测试报告见附件', 'plain', 'utf-8'))

# 添加附件就是加上一个MIMEBase，从本地读取一个图片:
with open('/Users/shixin/zuobiao/Report/坐标顶端功能.html', 'rb') as f:
    # 设置附件的MIME和文件名，这里是png类型:
    mime = MIMEBase('image', 'html', filename='坐标顶端功能')
    # 加上必要的头信息:
    mime.add_header('Content-Disposition', 'attachment', filename='坐标顶端功能.html')
